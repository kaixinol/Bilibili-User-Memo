...
- 默认头像（noface）用户可右键头像，手动填写头像 URL。
- 非默认头像用户可中键点击头像，手动填写头像 URL（可选同时关闭静默更新头像功能）。
- BilibiliID：点击可复制，复制后短暂显示“已复制”；ID 过长时悬停整个信息区可展开查看完整 ID。
...
- 搜索框支持昵称、备注、详细备注、ID 模糊搜索，输入后按 `Enter` 执行搜索。
- 搜索框使用浏览器原生表单记忆，历史建议由浏览器自动管理。
- 简体、繁体中文可互通搜索，例如用“备注”也能搜到“備註”。
- 右侧“×”按钮清空搜索。
...
- 多选模式下选择用户后，“一键刷新数据”会变为“刷新所选”，只刷新已选用户。
- 多选模式下未选择用户时，刷新按钮不可用。

### 添加用户

- 点击 `添加` 可手动添加用户。
- UID 与备注为必填项，UID 只支持数字。
- 头像 URL 可选；留空时会自动获取头像，若账号已注销则使用默认头像并标记为已注销。
